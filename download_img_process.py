...
if not os.path.exists('images_process'):
    os.makedirs('images_process')

def img_saver_process(url):
    response = requests.get(url)
    file_name = f'{url.split("/")[-1]}'
    with open(f'images_process/{file_name}', 'wb') as f:
...
if __name__ == '__main__':
    for url in URLS:
        process = Process(target=img_saver_process, args=(url,))
